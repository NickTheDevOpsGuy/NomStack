      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 space-y-2">
          {displayed.split(/(?<=\.)\s+/).map((step, i) => (
            <p
              key={i}
              className="break-words text-sm leading-relaxed text-slate-800 dark:text-sky-50"
            >
              {highlight(step, term)}
            </p>
          ))}
          ...
          {/* Example */}
          {sense.example && (
            <p className="border-l-2 border-sky-200 pl-3 text-sm italic text-slate-700 dark:border-sky-600 dark:text-sky-200">
              “{sense.example.trim()}”
            </p>
          )}
        </div>
...
function highlight(text: string, term: string) {
  ...
  return parts.map((p, i) =>
    i % 2 ? (
      <mark
        key={i}
        className="rounded bg-sky-200 px-0.5 dark:bg-sky-700/70"
      >
        {p}
      </mark>
    ) : (
